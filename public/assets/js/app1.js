...
function eventListeners() {
  // App Init
  document.addEventListener("DOMContentLoaded", driverView);
  // document.addEventListener("DOMContentLoaded", driverView);
...
function driverView() {
  ...
  if (currentJobs != ""){
    displayDiv.innerText= currentJobs 
  }
  if (displayDiv){
    displayDiv.innerText=  "its alive1";
  }
...
// print a job 1
function printJob1(pickUpObject) {
  console.log("job printed");
  console.log(pickUpObject);
  const displayDiv1 = document.createElement("div");
  displayDiv1.innerHTML= ` crew name: ${pickUpObject.crew} <br>
  vessel name: ${pickUpObject.vessel} <br>
  pick up location: ${pickUpObject.pickUp} <br>
 drop Off Location : ${pickUpObject.dropOff } <br>
 number of passengers : ${pickUpObject.numberCrew } <br>
remarks : ${pickUpObject.remarks } <br>
  Pick up date: ${pickUpObject.dateJa} :  ${pickUpObject.timeJa}  <br>
  `;
  document.querySelector("#display1-div").appendChild(displayDiv1);
}
// print a job 
function printJob(pickUpObject) {
  console.log("job printed");
  console.log(pickUpObject);
  printJob1(pickUpObject)
}
